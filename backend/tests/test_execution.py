...
from app.execution import (OrderStateError, closed_trades, mark, reconcile,
                           record_closed_trade, record_order,
                           stale_unknown_orders, transition)
...
def _reset(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(execution, "TRADES_LOG",
...
def test_ledger_record_and_mark():
    rec = record_order("o1", signal_id="s1", strategy_id="t",
                       symbol="X", side="buy", qty=10, entry=100.0,
                       broker="paper")
    assert rec["status"] == "PENDING"
    assert record_order("o1", signal_id="s1", strategy_id="t",
                        symbol="X", side="buy", qty=10, entry=100.0,
                        broker="paper")["id"] == "o1"  # idempotent
...
def test_stale_submitted_becomes_unknown():
    record_order("o2", signal_id=None, strategy_id="t", symbol="X",
                 side="buy", qty=1, entry=1.0, broker="kite")
    mark("o2", "SUBMITTED")
...
def test_reconcile_paper_ok_via_api():
    ...
    r = client.post("/api/orders", headers=h, json={
        "broker": "paper", "symbol": "X", "side": "buy", "qty": 1,
        "entry": 100.0, "targetPrice": 101.0, "stopLoss": 99.0})
    assert r.status_code == 200
    order_id = r.json()["result"]["orderId"]
    assert execution.get_order(order_id)["status"] == "FILLED"

    rep = client.post("/api/execution/reconcile", headers=h,
                      json={"broker": "paper"}).json()
    assert rep["ok"] is True
    assert risk.reconciliation_ok() is True


def test_reconcile_flags_unknown_and_closes_gate():
    record_order("o3", signal_id=None, strategy_id="t", symbol="X",
                 side="buy", qty=1, entry=1.0, broker="paper")
    mark("o3", "FILLED")
    mark("o3", "SUBMITTED") if False else None
    # force an unknown order (stale submitted)
    record_order("o4", signal_id=None, strategy_id="t", symbol="X",
                 side="buy", qty=1, entry=1.0, broker="paper")
    mark("o4", "SUBMITTED")
...
def test_record_closed_trade_one_position_one_trade():
    record_order("p1", signal_id="s1", strategy_id="rsi-reversal",
                 symbol="X", side="buy", qty=100, entry=100.0,
                 broker="paper")
    events = [
        {"id": "p1", "symbol": "X", "side": "buy", "qty": 50,
         "qty_total": 100, "entry": 100.0, "exit_price": 102.0,
         "exit": "TP1", "status": "open", "ts": 1.0},
        {"id": "p1", "symbol": "X", "side": "buy", "qty": 50,
         "qty_total": 100, "entry": 100.0, "exit_price": 98.0,
         "exit": "SL", "status": "closed", "ts": 2.0},
    ]
...
def test_record_closed_trade_without_final_ignored():
    events = [{"id": "p2", "symbol": "X", "side": "buy", "qty": 50,
               "qty_total": 100, "entry": 100.0, "exit_price": 102.0,
               "exit": "TP1", "status": "open", "ts": 1.0}]
...
def test_closed_trade_feeds_breaker():
    record_order("p3", signal_id=None, strategy_id="t", symbol="X",
                 side="buy", qty=10, entry=100.0, broker="paper")
    events = [{"id": "p3", "symbol": "X", "side": "buy", "qty": 10,
               "qty_total": 10, "entry": 100.0, "exit_price": 95.0,
               "exit": "SL", "status": "closed", "ts": 2.0}]
