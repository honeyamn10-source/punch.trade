...
_ALLOWED_METRICS = (
    "win_rate", "loss_rate", "profit_factor", "net_pnl", "expectancy",
    "max_drawdown_pct", "max_consecutive_losses", "sharpe", "trades",
    "gross_profit", "gross_loss", "commission", "equity_final",
    "avg_win", "avg_loss",
)
_ALLOWED_STATUS = (
    "status", "reason", "score", "canPromoteTo", "id", "name", "symbol",
)
...
def detect_model() -> Dict:
    ...
    try:
        ...
        qwen = sorted([m for m in lines if m.startswith("qwen2.5")],
                      key=lambda m: _size_rank(m), reverse=True)
        if not qwen:
            return {"model": None,
...
def build_prompt(strategy: Dict, research: Optional[Dict],
                 status: Optional[Dict], drift: Optional[Dict]) -> str:
    """Whitelist-only prompt: metrics + scores + counts, nothing sensitive."""
    ctx: Dict = {
        "strategy": _pick(strategy, ("id", "name", "symbol", "timeframe",
                                     "family", "version", "status")),
        "metrics": _pick((research or {}).get("metrics") or {},
                         _ALLOWED_METRICS),
        "qualityGate": _pick((research or {}).get("qualityGate") or {},
                             ("passed", "score", "reasons", "sample")),
        "parameterStability": _pick((research or {}).get("parameterStability") or {},
                                    ("passed", "score", "worst")),
        "walkForward": _pick((research or {}).get("walkForward") or {},
                             ("consistency", "windows")),
        "bootstrap": _pick((research or {}).get("bootstrap") or {},
                           ("meanExpectancy", "stdExpectancy", "error")),
...
def analyze(strategy: Dict, research: Optional[Dict] = None,
    ...
    try:
        ...
        if not model:
            return {"model": None, "analysis": None,
        ...
        if not text:
            return {"model": model, "analysis": None,
                    "error": "empty model response", "elapsedSec": 0.0}
        return {"model": model, "analysis": security.sanitize(text),
                "error": None,
                "elapsedSec": round(time.time() - started, 2)}
...
def status() -> Dict:
    info = detect_model()
    return {"enabled": bool(info["model"]), "model": info["model"],
            "host": _ollama_host(), "reason": info["reason"]}