...
FILL_REASONS = ("ENTRY", "TP1", "TP2", "TP3", "STOP", "MANUAL", "END_OF_TEST", "LIQUIDATION")


class Fill:
    __slots__ = ("ts", "reason", "price", "qty", "commission")

    def __init__(self, ts: float, reason: str, price: float, qty: float, commission: float = 0.0):
...
class CompletedTrade:
    ...
    def __init__(
        ...
        self.regime = regime
        self.fills: list[Fill] = [Fill(entry_ts, "ENTRY", entry_price, qty, entry_commission)]
        self.exit_ts: float | None = None
    ...
    # --------------------------------------------------------- fills ----
    def add_fill(
        self, ts: float, reason: str, price: float, qty: float, commission: float = 0.0
    ) -> None:
    ...
    def close(
        ...
        self.net_pnl = pnl_mod.trade_pnl(
            self.side, self.entry_price, avg_exit, self.qty, self.commission
        )
        self.net_pnl_pct = pnl_mod.trade_pnl_pct(self.entry_price, avg_exit, self.side)
        return self.to_dict()
