...
class CCXTBroker(BrokerAdapter):
    ...
    def status(self) -> Dict:
        try:
            ...
            return {"broker": "binance", "connected": True,
                    "testnet": self.testnet,
                    "account": "spot",
                    "usdt": round(float(balance.get("USDT", {}).get("free", 0) or 0), 2)}
        except Exception as e:
            raise BrokerError(f"Binance auth failed: {e}")
    ...
    def get_historical_bars(self, symbol: str, interval: str, days: int) -> List[dict]:
        ...
        try:
            ...
            raise BrokerError(f"Binance OHLCV failed: {e}")
        return [{"ts": r[0] / 1000, "open": r[1], "high": r[2],
    ...
    def place_bracket(self, symbol: str, side: str, qty: int,
        ...
        try:
            entry_order = self._ex.create_order(symbol, "market" if market else "limit",
                                                side_s, qty, None if market else (price or entry))
            entry_id = entry_order.get("id", "?")
            tp = self._ex.create_order(symbol, "take_profit_limit", exit_side, qty,
                                       target, {"stopPrice": target})
            sl = self._ex.create_order(symbol, "stop_loss_limit", exit_side, qty,
                                       stop, {"stopPrice": stop})
        except Exception as e:
            raise BrokerError(f"Binance bracket failed: {e}")
        return {"orderId": entry_id, "status": "ACCEPTED", "broker": "binance",
                "legs": [
                    {"leg": "ENTRY", "orderId": entry_id, "status": entry_order.get("status", "?"), "price": entry_order.get("price")},
                    {"leg": "TAKE_PROFIT", "orderId": tp.get("id", "?"), "status": tp.get("status", "?"), "price": target},
                    {"leg": "STOP_LOSS", "orderId": sl.get("id", "?"), "status": sl.get("status", "?"), "price": stop},
                ]}
    ...
    def get_positions(self) -> List[Dict]:
        ...
        try:
            ...
            raise BrokerError(f"Binance orders failed: {e}")
        out = []
        for o in orders:
            out.append({"id": o.get("id"), "symbol": o.get("symbol"),
                        "side": o.get("side"), "qty": o.get("amount"),
                        "price": o.get("price") or o.get("stopPrice"),
                        "type": o.get("type"), "status": "open"})
        usdt = balance.get("USDT", {}).get("free", 0) or 0
        return out + [{"id": "balance", "symbol": "USDT", "qty": round(float(usdt), 2),
                       "side": "available", "status": "balance"}]

    def get_fills(self, since: Optional[float] = None) -> List[Dict]:
        try:
            trades = self._ex.fetch_my_trades()
        except Exception as e:
            raise BrokerError(f"Binance trades failed: {e}")
        out = []
        for t in trades:
            ts = t.get("timestamp")
            if ts is not None and since is not None and ts / 1000 < since:
                continue
            out.append({"id": t.get("id"), "symbol": t.get("symbol"),
                        "side": t.get("side"), "qty": t.get("amount"),
                        "price": t.get("price"),
