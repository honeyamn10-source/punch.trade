...
def login_url(api_key: str) -> str:
    if KiteConnect is None:
        raise BrokerError("kiteconnect not installed. pip install kiteconnect[ws]") from None
...
def generate_session(api_key: str, api_secret: str, request_token: str) -> dict:
    ...
    try:
        ...
        raise BrokerError(f"Kite session failed: {e}") from None
...
class KiteAdapter(BrokerAdapter):
    ...
    def _resolve_token(self, symbol: str) -> int:
        ...
        raise BrokerError(f"Symbol {symbol} not found on NSE") from None
    ...
    def status(self) -> dict:
        try:
            ...
            raise BrokerError(f"Kite session invalid or expired: {e}") from None
    ...
    def get_historical_bars(self, symbol: str, interval: str, days: int) -> list[dict]:
        ...
        try:
            ...
            raise BrokerError(f"Kite historical data failed: {e}") from None
    ...
    def place_bracket(
        ...
        self._resolve_token(symbol)  # validates the instrument exists
        ...
        try:
            ...
            raise BrokerError(f"Kite order rejected: {e}") from None
        return {
            "orderId": order_id,
            "status": "ACCEPTED",
            "broker": "kite",
    ...
    def get_positions(self) -> list[dict]:
        try:
            ...
            raise BrokerError(f"Kite positions failed: {e}") from None
        out = []
            ...
            out.append(
                {
                    ...
                    "pnl_pct": round(
                        (row.get("last_price", 0) - row.get("average_price", 0))
                        / row.get("average_price", 1)
                        * 100,
                        2,
                    )
    ...
    def get_fills(self, since: float | None = None) -> list[dict]:
        try:
            ...
            raise BrokerError(f"Kite trades failed: {e}") from None
        out = []
        for t in trades:
