...
class PaperBroker(BrokerAdapter):
    ...
    def place_bracket(
        ...
        position = {
            ...
            "target": round(tps[0], 2),
            "targets": [round(t, 2) for t in tps],
            "stop": round(stop, 2),
            "status": "open",
            "opened_at": time.time(),
            "pnl_pct": 0.0,
    ...
    def _exit_fraction(
        self, p: dict, price: float, label: str, qty: float, closed: list[dict]
    ) -> None:
        p["remaining_qty"] -= qty
